...
> **현재 상태:** 기획 중. 전투·상점·런 구조의 큰 틀이 잡혔고 구현은 시작 전이다. 장르·방향은 [GAME_DESIGN.md](docs/game_design/GAME_DESIGN.md)에만 적는다 — **오토배틀러가 아니라 정비 단계와 전투 라운드를 오가는 로그라이크 타워디펜스다** (판단은 정비에서, 전투 중 입력은 스킬카드뿐). 다음에 무엇을 정해야 하는지는 그 문서의 「미결」에 모여 있다.
>
> 툴링(스킬·에이전트·컨벤션)은 TheSevenAutoBattle에서 가져온 골격이라, 오토배틀러/오토체스 전제가 남은 서술이 곳곳에 있다. `prototypes/`에는 현재 설계의 프로토(`realtime_defense`) 하나만 남아 있다 — 앞선 두 프로토는 설계와 어긋나 삭제했다(git 히스토리에 있다).

---

## 작업 규칙

- **사용자가 확정하지 않은 내용은 기획서에 절대 넣지 않는다.**
  - 제안·추론·"이렇게 하면 좋겠다"는 **대화에서만** 한다. 사용자가 명시적으로 확정한 뒤에야 `docs/`에 적는다.
  - 시스템 이름·용어를 새로 만들어 문서에 심지 않는다. 사용자가 그 단어를 쓴 적이 없으면 그건 확정된 게 아니다.
  - 확정 여부가 애매하면 **적지 말고 묻는다.** 애매한 채로 적어두고 나중에 지우는 것보다, 안 적고 다시 묻는 쪽이 항상 싸다.
  - **Why:** 기획서의 모든 서술은 사용자가 내린 결정이어야 한다. Claude가 지어낸 시스템·용어가 섞이면 사용자가 자기 게임 문서를 읽으면서 "이건 내가 정한 적 없는데"를 겪게 되고, 그 시점부터 문서 전체를 신뢰할 수 없게 된다.
