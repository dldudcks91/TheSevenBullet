...
> **현재 상태:** 기획 중. 전투·상점·런 구조의 큰 틀이 잡혔고 구현은 시작 전이다. 장르·방향은 [GAME_DESIGN.md](docs/game_design/GAME_DESIGN.md)에만 적는다 — **오토배틀러가 아니라 실시간 로그라이크 타워디펜스다** (라운드·정비 단계 없음, 모든 판단이 실시간). 다음에 무엇을 정해야 하는지는 그 문서의 「미결」에 모여 있다.
>
